...
const jwt = require('jsonwebtoken'); // Ajouter JWT pour la gestion des tokens
...
const JWT_SECRET = '1234455444AZERRTTYUUUYTRE';
...
// Route pour créer un item avec JWT
...
app.put('/item/:id', verifyToken, upload.single('image'), (req, res) => {
  const itemId = req.params.id;
...
app.post('/update-image/:id', verifyToken, upload.single('image'), (req, res) => {
  const { id } = req.params;
  const image = req.file ? `/uploads/${req.file.filename}` : null;
